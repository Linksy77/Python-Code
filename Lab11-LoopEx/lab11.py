...
def insertV1(L, i):
    ...
    posOfunSorted = i
    while True:
        if posOfunSorted > 0 and L[posOfunSorted] < L[posOfunSorted - 1]:
            swap(L, posOfunSorted, (posOfunSorted - 1))
            posOfunSorted -= 1
        else:
            break
...
def search(L, i, x):
    ...
    j = 0
    unsortedNums = L[:i+1]
    for num in unsortedNums:
        if len(unsortedNums) == 1:
            if x < num:
                return j
            else:
                return j + 1
        elif x >= num and j <= (i-1):
            j += 1
        else:
            return j
...
def letterCounts(fname):
    ...
    charCount = {'a': 0, 'b': 0, 'c': 0, 'd': 0, 'e': 0, 'f': 0, 'g': 0,
                 'h': 0, 'i': 0, 'j': 0, 'k': 0, 'l': 0, 'm': 0, 'n': 0,
                 'o': 0, 'p': 0, 'q': 0, 'r': 0, 's': 0, 't': 0, 'u': 0,
                 'v': 0, 'w': 0, 'x': 0, 'y': 0, 'z': 0, 'non-letter': 0}
    ...
    with open(fname, 'r') as file:
        ...
        sortedLetts = sorted(charCount.items(), key = lambda x : x[1], reverse = True)
        
        finalLetts = []
        
        for lett in sortedLetts:
            if lett[1] != 0:
                finalLetts.append((lett[1], lett[0]))

        for lett in finalLetts:
            print(lett)
